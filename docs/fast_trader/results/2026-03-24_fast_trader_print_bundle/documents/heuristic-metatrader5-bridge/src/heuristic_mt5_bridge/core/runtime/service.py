...
class CoreRuntimeConfig:
    ...
    def load(cls, repo_root: Path) -> "CoreRuntimeConfig":
        ...
        return cls(
            ...
            sessions_enabled=_parse_bool(getenv("BROKER_SESSIONS_ENABLED", env_values, "true"), True),
            ...
            indicator_enabled=_parse_bool(getenv("INDICATOR_ENRICHMENT_ENABLED", env_values, "true"), True),
...
class CoreRuntimeService:
    ...
    async def _refresh_symbol_catalog(self) -> None:
        ...
        for entry in self.symbol_catalog:
            await asyncio.to_thread(upsert_symbol_catalog_cache, self.config.runtime_db_path, entry)
    ...
    def _initialize_bootstrap_universe(self) -> None:
        bootstrap_result = self.subscription_manager.bootstrap_from_env(self.config.watch_symbols)
    ...
    async def bootstrap(self) -> None:
        ...
        self.risk_kernel = RiskKernel.from_env(
            db_path=self.config.runtime_db_path,
            broker_server=str(self.broker_identity.get("broker_server", "")).strip(),
            account_login=int(self.broker_identity.get("account_login", 0) or 0),
        )
        ...
        self._initialize_bootstrap_universe()
        await self._refresh_symbol_specs()
...
async def build_runtime_service(repo_root: Path) -> CoreRuntimeService:
    ...
    smc_enabled = os.getenv("SMC_SCANNER_ENABLED", "").strip().lower() in ("1", "true", "yes", "on")
    if smc_enabled:
    ...
    fast_enabled = (
        os.getenv("FAST_TRADER_ENABLED", "").strip().lower() in ("1", "true", "yes", "on")
        or os.getenv("FAST_DESK_ENABLED", "").strip().lower() in ("1", "true", "yes", "on")
    )
    if fast_enabled:
