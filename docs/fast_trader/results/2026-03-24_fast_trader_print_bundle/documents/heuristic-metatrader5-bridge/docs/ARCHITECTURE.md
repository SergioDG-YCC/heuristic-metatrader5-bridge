...
> **Status**: implemented and tested — 79/79 tests passing (March 2026)
...
1. `FastContextService` builds H1 bias and hard gates (`session`, `spread`, `slippage`, `stale_feed`, regime).
2. `FastSetupEngine` detects M5 setups (`3 core + 4 patterns`).
...
graph TD
    ...
    subgraph FD["Fast Desk  FAST_DESK_ENABLED=true"]
        ...
        FD_scan --> FD_risk["FastRiskEngine\nlot-size · drawdown guard"]
        FD_risk --> FD_policy["FastEntryPolicy\nno duplicate · max positions"]
        FD_policy --> FD_worker["FastSymbolWorker × symbol\nscan 5s · custody 2s"]
        AS --> FD_cust["FastCustodian\ntrail SL · lock profit · close"]
        FD_cust --> FD_worker
        FD_worker -->|send_execution_instruction| Conn
        FD_worker -->|modify_position_levels·close_position| Conn
        FD_worker --> DB
    end
...
sequenceDiagram
    ...
    participant Scan as FastScannerService
    ...
    loop every scan_interval (5s)
        W->>Scan: scan_symbol(candles, pip_size)
        Scan-->>W: FastSignal | None

        alt signal exists and cooldown elapsed
            W->>Risk: check_account_safe()
            W->>Risk: calculate_lot_size()
            W->>Policy: can_open(symbol, side, open_positions)
            alt allowed
                W->>Exec: open_position(connector, signal, volume)
                W->>DB: upsert_fast_signal()
            end
        end
    end

    loop every custody_interval (2s)
        W->>Cust: evaluate(position, current_price, pip_size)
        alt TRAIL_SL
            W->>Exec: apply_custody(TRAIL_SL, new_sl, position=pos)
            Exec->>Conn: modify_position_levels(symbol, position_id, stop_loss, tp=None)
        else CLOSE
            W->>Exec: apply_custody(CLOSE, position=pos)
