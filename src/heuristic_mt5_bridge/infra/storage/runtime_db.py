...
def ensure_runtime_db(db_path: Path) -> None:
    ...
    with runtime_db_connection(db_path) as conn:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS symbol_subscription_state (
                broker_server TEXT NOT NULL,
                account_login INTEGER NOT NULL,
                symbol TEXT NOT NULL,
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS symbol_desk_assignment_state (
                broker_server TEXT NOT NULL,
                account_login INTEGER NOT NULL,
                symbol TEXT NOT NULL,
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_symbol_subscription_state_updated_at "
            "ON symbol_subscription_state(broker_server, account_login, updated_at DESC)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_symbol_desk_assignment_state_updated_at "
            "ON symbol_desk_assignment_state(broker_server, account_login, updated_at DESC)"
        )
        conn.execute(
...
def batch_upsert_market_state_cache(
    db_path: Path,
    rows: list[dict[str, Any]],
) -> None:
    """Insert/update all market state checkpoint rows in a single transaction.

    Each dict in ``rows`` must have keys: broker_server, account_login,
    symbol, timeframe, updated_at, state_summary, chart_context,
    indicator_summary, source.

    Replaces the per-row loop that opened N connections per checkpoint cycle
    (N = subscribed_symbols × watch_timeframes).
    """
    if not rows:
        return
    with runtime_db_connection(db_path) as conn:
        conn.executemany(
            ...
            [
                (
                    str(row["broker_server"]).strip(),
                    int(row["account_login"]),
                    str(row["symbol"]).upper(),
                    str(row["timeframe"]).upper(),
                    row["updated_at"],
                    json_text(row["state_summary"]),
                    json_text(row["chart_context"]) if row.get("chart_context") is not None else None,
                    json_text(row["indicator_summary"]) if row.get("indicator_summary") is not None else None,
                    row["source"],
                )
                for row in rows
            ],
...
def batch_upsert_symbol_catalog_cache(db_path: Path, entries: list[dict[str, Any]]) -> None:
    """Insert/update all catalog entries in a single connection and transaction.

    Replaces the per-symbol loop that opened ~6000 connections for a typical
    MT5 catalog (3000 symbols × 2 connections per call).
    """
    if not entries:
        return
    with runtime_db_connection(db_path) as conn:
...
def save_symbol_subscription_snapshot(
    db_path: Path,
    *,
    broker_server: str,
    account_login: int,
    ...
    current_symbols = {str(symbol).upper() for symbol in subscribed_symbols if str(symbol).strip()}
    with runtime_db_connection(db_path) as conn:
        ...
        known_symbols = current_symbols | {str(row[0]).upper() for row in existing_rows if row and row[0]}
        for symbol in sorted(known_symbols):
            conn.execute(
                """
                INSERT INTO symbol_subscription_state (
                    broker_server,
                    account_login,
                    symbol,
                    is_subscribed,
                ...
                (
                    ...
                    int(account_login),
                    symbol,
                    1 if symbol in current_symbols else 0,
                    str(source).strip() or None,
                    now,
                ),
            )
        conn.commit()
...
def load_symbol_subscription_states(
    db_path: Path,
    *,
    broker_server: str,
    account_login: int,
    ...
    for row in rows:
        item = dict(row)
        item["symbol"] = str(item.get("symbol", "")).upper()
        item["is_subscribed"] = bool(int(item.get("is_subscribed", 0) or 0))
        items.append(item)
    return items


def upsert_symbol_desk_assignment_state(
    db_path: Path,
    *,
    broker_server: str,
    account_login: int,
    ...
    ensure_runtime_db(db_path)
    normalized_desks = sorted(
        {
            str(desk).strip().lower()
            for desk in desks
            if str(desk).strip().lower() in {"fast", "smc"}
        }
    )
    now = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    with runtime_db_connection(db_path) as conn:
        conn.execute(
            """
            INSERT INTO symbol_desk_assignment_state (
                broker_server,
                account_login,
                symbol,
                desks_json,
            ...
            ON CONFLICT(broker_server, account_login, symbol) DO UPDATE SET
                desks_json=excluded.desks_json,
                updated_at=excluded.updated_at
            """,
            (
                str(broker_server).strip(),
...
def load_symbol_desk_assignment_states(
    db_path: Path,
    *,
    broker_server: str,
    account_login: int,
    ...
    with runtime_db_connection(db_path) as conn:
        ...
        rows = conn.execute(
            """
            SELECT *
            FROM symbol_desk_assignment_state
            WHERE broker_server = ? AND account_login = ?
            ORDER BY symbol ASC
            """,
            (str(broker_server).strip(), int(account_login)),
    ...
    for row in rows:
        item = dict(row)
        item["symbol"] = str(item.get("symbol", "")).upper()
        item["desks"] = decode_json_text(item.get("desks_json"), [])
        items.append(item)
    return items
