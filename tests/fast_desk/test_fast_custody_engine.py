...
def _ctx(h1_bias: str = "buy") -> FastContext:
    return FastContext(
        symbol="EURUSD",
        session_name="london",
        h1_bias=h1_bias,
...
def test_custody_no_passive_underwater_closes_on_opposite_h1_bias() -> None:
    engine = FastCustodyEngine(FastCustodyPolicyConfig(hard_cut_r=5.0))
    decision = engine.evaluate_position(
        position={
            "position_id": 103,
            "side": "buy",
            "price_open": 1.1000,
            "price_current": 1.0992,
            "stop_loss": 1.0988,
            "volume": 0.10,
        },
        candles_m1=_candles(),
